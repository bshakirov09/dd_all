            ...
            if ($stripeToken->created > 0) {
                if (empty($input['delivery_address_id'])) {
                    $order = $this->orderRepository->create(
                        $request->only('user_id', 'order_status_id', 'tax', 'hint', 'start_time')
                    );
                } else {
                    $order = $this->orderRepository->create(
                        $request->only('user_id', 'order_status_id', 'tax','options', 'delivery_address_id', 'delivery_fee', 'hint', 'start_time')
                    );
                }
...
//                $amountWithTax = $amount + ($amount * $order->tax / 100);
    ...
    private function cashPayment(Request $request){
        ...
        try {
            $order = $this->orderRepository->create(
                $request->only('user_id', 'order_status_id', 'tax', 'delivery_address_id', 'delivery_fee', 'hint', 'start_time')
            );
            Log::info($order);
