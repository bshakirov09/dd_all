...
class MarketAPIController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $market->toArray();
        $market['gallery'] = '';
        return $this->sendResponse($market, 'Market retrieved successfully');
    }
     ...
     public function store(Request $request)
    {
        ...
        $input = $request->all();
        $user = \App\Models\User::query()->where('api_token', $request->api_token)->firstOrFail();
        $input['users'] = [$user->id];
//        dd($input['users']);
...
//        $customFields = $this->customFieldRepository->findByField('custom_field_model', $this->marketRepository->model());
        try {
            ...
            if (isset($input['additional_image']) && $input['additional_image']) {
                $cacheUpload = $this->uploadRepository->getByUuid($input['additional_image']);
                $mediaItem = $cacheUpload->getMedia('additional_image')->first();
                $mediaItem->copy($market, 'additional_image');
            }
