...
//                }
    ...
    public function index($type = null, $tab = null)
    {
        ...
        $banner_upload = $this->uploadRepository->findByField('uuid', setting('additional_image'))->first();

        $currencies = $this->currencyRepository->all()->pluck('name_symbol', 'id');

        $customFieldModels = getModelsClasses(app_path('Models'));

        return view('settings.' . $type . '.' . $tab . '', compact(['languages', 'type', 'tab', 'langFiles', 'timezones', 'banner_upload', 'upload', 'customFieldModels', 'currencies', 'mobileLanguages', 'containsUpdate']));
    }
