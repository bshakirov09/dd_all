...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        try {
            ...
            config(['mail.host' => setting('mail_host', 'smtp.netcorecloud.net')]);
            config(['mail.port' => setting('mail_port', 25)]);
            config(['mail.encryption' => setting('mail_encryption', 'tls')]);
            config(['mail.username' => setting('mail_username', 'lightningraces@pepisandbox.com')]);
            config(['mail.password' => setting('mail_password', 'Int3g321!')]);
            config(['mail.from.address' => setting('mail_from_address', 'lightningraces@pepisandbox.com')]);
            config(['mail.from.name' => setting('mail_from_name', 'lightningraces')]);
