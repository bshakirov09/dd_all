        ...
        <script type="text/javascript">
            ...
            @if(isset($market) && $market->hasMedia('image'))
                ...
                collection_name: "{!! $market->getFirstMedia('image')->collection_name !!}"
                    ...
                    @endif
            var dz_var15671147011688676454ble = $("#image").dropzone({
...
<!-- Additional Image Field -->
    <div class="form-group row">
        {!! Form::label('additional_image', 'Additional image', ['class' => 'col-3 control-label text-right']) !!}
        <div class="col-9">
            <div style="width: 100%" class="dropzone image" id="additional_image" data-field="additional_image">
                <input type="hidden" name="additional_image">
            </div>
            <a href="#loadMediaModal" data-dropzone="additional_image" data-toggle="modal" data-target="#mediaModal" class="btn btn-outline-{{setting('theme_color','primary')}} btn-sm float-right mt-1">{{ trans('lang.media_select')}}</a>
            <div class="form-text text-muted w-50">
                {{ trans("lang.market_image_help") }}
            </div>
        </div>
    ...
    @prepend('scripts')
        <script type="text/javascript">
            // additional_image
            var additional_var15671147011688676454ble = '';
            @if(isset($market) && $market->hasMedia('additional_image'))
                additional_var15671147011688676454ble = {
                name: "{!! $market->getFirstMedia('additional_image')->name !!}",
                size: "{!! $market->getFirstMedia('additional_image')->size !!}",
                type: "{!! $market->getFirstMedia('additional_image')->mime_type !!}",
                collection_name: "{!! $market->getFirstMedia('additional_image')->collection_name !!}"
            };
            @endif
            var additional_dz_var15671147011688676454ble = $("#additional_image").dropzone({
                url: "{!!url('uploads/store')!!}",
                addRemoveLinks: true,
                maxFiles: 1,
                init: function () {
                    @if(isset($market) && $market->hasMedia('additional_image'))
                    dzInit(this, additional_var15671147011688676454ble, '{!! url($market->getFirstMediaUrl('additional_image','thumb')) !!}')
                    @endif
                },
                accept: function (file, done) {
                    dzAccept(file, done, this.element, "{!!config('medialibrary.icons_folder')!!}");
                ...
                maxfilesexceeded: function (file) {
                    additional_dz_var15671147011688676454ble[0].mockFile = '';
                    dzMaxfile(this, file);
                },
                complete: function (file) {
                    dzComplete(this, file, additional_var15671147011688676454ble, additional_dz_var15671147011688676454ble[0].mockFile);
                    additional_dz_var15671147011688676454ble[0].mockFile = file;
                },
                removedfile: function (file) {
                    dzRemoveFile(
                        file, additional_var15671147011688676454ble, '{!! url("markets/remove-media") !!}',
                        'additional_image', '{!! isset($market) ? $market->id : 0 !!}', '{!! url("uplaods/clear") !!}', '{!! csrf_token() !!}'
                    );
                }
            });
            additional_dz_var15671147011688676454ble[0].mockFile = additional_var15671147011688676454ble;
            dropzoneFields['additional_image'] = additional_dz_var15671147011688676454ble;
        </script>
@endprepend
