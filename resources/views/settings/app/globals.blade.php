    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                <!-- App Banner Field -->
                    <div class="form-group row">
                        {!! Form::label('app_banner', 'App banner', ['class' => 'col-4 control-label text-right']) !!}
                        <div class="col-8">
                            <div style="width: 100%" class="dropzone additional_image" id="additional_image" data-field="additional_image">
                                <input type="hidden" name="additional_image">
                            </div>
                            <a href="#loadMediaModal" data-dropzone="additional_image" data-toggle="modal" data-target="#mediaModal" class="btn btn-outline-{{setting('theme_color')}} btn-sm float-right mt-1">{{ trans('lang.media_select')}}</a>
                            <div class="form-text text-muted w-50">
                                Upload your banner or select from media library
                            </div>
                        </div>
                    </div>
                    ...
                    @prepend('scripts')
                        <script type="text/javascript">
                            var banner_dzAppLogo = '';
                            @if(isset($banner_upload) && $banner_upload->hasMedia('additional_image'))
                                banner_dzAppLogo = {
                                name: "{!! $banner_upload->getFirstMedia('additional_image')->name !!}",
                                size: "{!! $banner_upload->getFirstMedia('additional_image')->size !!}",
                                type: "{!! $banner_upload->getFirstMedia('additional_image')->mime_type !!}",
                                collection_name: "{!! $banner_upload->getFirstMedia('additional_image')->collection_name !!}"
                            };
                            @endif
                            var banner_dz_dzAppLogo = $(".dropzone.additional_image").dropzone({
                                url: "{!!url('uploads/store')!!}",
                                addRemoveLinks: true,
                                maxFiles: 1,
                                init: function () {
                                    @if(isset($banner_upload) && $banner_upload->hasMedia('additional_image'))
                                    dzInit(this, banner_dzAppLogo, '{!! url($banner_upload->getFirstMediaUrl('additional_image','thumb')) !!}')
                                    @endif
                                },
                                accept: function (file, done) {
                                    dzAccept(file, done, this.element, "{!!config('medialibrary.icons_folder')!!}");
                                ...
                                maxfilesexceeded: function (file) {
                                    banner_dz_dzAppLogo[0].mockFile = '';
                                    dzMaxfile(this, file);
                                },
                                complete: function (file) {
                                    dzComplete(this, file, banner_dzAppLogo, banner_dz_dzAppLogo[0].mockFile);
                                    banner_dz_dzAppLogo[0].mockFile = file;
                                },
                                removedfile: function (file) {
                                    dzRemoveFile(
                                        file, banner_dzAppLogo, '{!! url("uploads/remove-media") !!}',
                                        'additional_image', '{!! isset($banner_upload) ? $banner_upload->id : 0 !!}', '{!! url("uplaods/clear") !!}', '{!! csrf_token() !!}'
                                    );
                                }
                            });
                            banner_dz_dzAppLogo[0].mockFile = banner_dzAppLogo;
                            dropzoneFields['additional_image'] = banner_dz_dzAppLogo;
                        </script>
                @endprepend
